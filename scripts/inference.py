...
sys.path.append("/workspace/code/fusion-diffusion3.0")
import cv2
import torch
import numpy as np
...
def normalize_img(input):
    return ((input - input.min())/(input.max() - input.min())) * 2.0 - 1.0

def denormalize_img(input, a, b):
    return ((input + 1.0)/2.0) * a + b
...
def main():
    ...
    opt.outdir = opt.dir + "scripts/results/"

    opt.ckpt = opt.dir + "checkpoints/pbe_model.ckpt"
    if opt.model_name != 'None':
        opt.delta_ckpt = opt.dir + "models/fusion-diffusion3.0/" + opt.model_name + "/checkpoints/delta_last.ckpt"
    else:
        opt.delta_ckpt = None

    opt.image_path = opt.dir + "examples/image/" + opt.example_img + "_img.png"
    opt.mask_path = opt.dir + "examples/mask/" + opt.example_img + "_mask.png"

    opt.reference_path = os.path.join(opt.id_dir, opt.id_class, "test.jpg")

    opt.point_ROI = opt.dir + "examples/image/" + opt.example_roi
    ...
    if opt.delta_ckpt is not None:
        delta_st = torch.load(opt.delta_ckpt, map_location='cuda:0')
    ...
    if opt.high_r == True:
        bbox_list = []
        with open(opt.point_ROI) as f:
            line = f.readline()
            while line:
                line_split = line.strip('\n').split(" ")
                bbox_list.append(int(line_split[0]))
                line = f.readline()
        bbox = bbox_list
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                image_tensor_backup = image_tensor.unsqueeze(0)
                if opt.high_r == True:
                    image_tensor = image_tensor[0:3, bbox[1]:bbox[3], bbox[0]:bbox[2]]
                ...
                mask_tensor_backup = mask_tensor
                if opt.high_r == True:
                    mask_tensor = mask_tensor[0:1, 0:1, bbox[1]:bbox[3], bbox[0]:bbox[2]]
                inpaint_image = image_tensor*mask_tensor
                inpaint_image_backup = image_tensor_backup*mask_tensor_backup
                test_model_kwargs={}
                ...
                # x_samples_ddim = denormalize_img(x_samples_ddim, a, b)

                if opt.high_r == True:
                    x_samples_ddim = inpaint_image.to(device) + (1 - mask_tensor.to(device)) * x_samples_ddim

                    img_hr = copy.deepcopy(image_tensor_backup)
                    img_hr[:, :, bbox[1]:bbox[3], bbox[0]:bbox[2]] = x_samples_ddim
                    x_samples_ddim = img_hr
                ...
                if not opt.skip_save:
                    for i,x_sample in enumerate(x_checked_image_torch):
                        ...
                        all_img.append(un_norm(image_tensor_backup[i]).cpu())
                        all_img.append(un_norm(inpaint_image_backup[i]).cpu())
                        ref_img=ref_tensor
                        opt.H = image_tensor_backup[i].shape[1]
                        opt.W = image_tensor_backup[i].shape[2]
                        ...
                        img.save(os.path.join(grid_path, opt.id_class+ '_' + 'grid-'+filename[:-4]+'_'+str(opt.seed)+'.png'))

                        x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')

                        # 边缘平滑过度 mask形态学梯度变换+平滑滤波
                        # ---------------------------------------------------------------------------------------------------------
                        if opt.high_r == True:
                            x_sample_smo = cv2.GaussianBlur(x_sample, (7, 7), 0, 0)
                            mask_numpy = rearrange(mask_tensor_backup.squeeze(0).cpu().numpy(), 'c h w -> h w c')
                            mask_dst = np.expand_dims(cv2.morphologyEx(mask_numpy, cv2.MORPH_GRADIENT, np.ones((7, 7), np.uint8)), -1)
                            image_dst = x_sample_smo * mask_dst
                            x_sample = image_dst + x_sample * (1 - mask_dst)

                        img = Image.fromarray(x_sample.astype(np.uint8))
                        img = put_watermark(img, wm_encoder)
                        img.save(os.path.join(result_path, opt.id_class+ '_' + filename[:-4]+'_'+str(opt.seed)+".png"))
                        ...
                        mask_save.save(os.path.join(sample_path, opt.id_class+ '_' + filename[:-4]+'_'+str(opt.seed)+"_mask.png"))

                        GT_img=255.*rearrange(all_img[0], 'c h w -> h w c').numpy()
                        GT_img = Image.fromarray(GT_img.astype(np.uint8))
                        GT_img.save(os.path.join(sample_path, opt.id_class+ '_' + filename[:-4]+'_'+str(opt.seed)+"_GT.png"))

                        inpaint_img=255.*rearrange(all_img[1], 'c h w -> h w c').numpy()
                        inpaint_img = Image.fromarray(inpaint_img.astype(np.uint8))
                        inpaint_img.save(os.path.join(sample_path, opt.id_class+ '_' + filename[:-4]+'_'+str(opt.seed)+"_inpaint.png"))

                        ref_img=255.*rearrange(all_img[2], 'c h w -> h w c').numpy()
                        ref_img = Image.fromarray(ref_img.astype(np.uint8))
                        ref_img.save(os.path.join(sample_path, opt.id_class+ '_' + filename[:-4]+'_'+str(opt.seed)+"_ref.png"))
