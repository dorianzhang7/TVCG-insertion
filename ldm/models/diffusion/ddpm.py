...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        ...
        return x,inpaint,mask,reference,bbox_list
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x,inpaint,mask,reference,bbox_list = super().get_input(batch, k)
        if bs is not None:
            x = x[:bs]
            inpaint = inpaint[:bs]
            mask = mask[:bs]
            reference = reference[:bs]
        # 分别提取出datase的输出信息 字典形式
        x = x.to(self.device)
        # 将x_0 （1*3*512*512）输入encoder 得到1*4*64*64
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
        # 将inpaint向量 （1 * 3 * 512 * 512）输入encoder 得到1 * 4 * 64 * 64
        ...
        # 将mask向量 resize 得到1*1*64*64  注意 这里mask是直接resize维度 没有经过encoder
        mask_resize = Resize([z.shape[-1],z.shape[-1]])(mask)
        # 将三者维度拼接 得到 1*9*64*64 作为unet的输入  其中 x_t:1*4*64*64(是原始图像和随机噪声的混合分布)  inpaint:1*4*64*64  mask:1*1*64*64
        ...
        out = [z_new, c, bbox_list]  # 这里传进入的c是refer image
    ...
    def shared_step(self, batch, **kwargs):
        x, c, bbox_list = self.get_input(batch, self.first_stage_key)
        c_dict = {'c_image':c, 'bbox_list': bbox_list}
        loss = self(x, c_dict)
        return loss

    def forward(self, x, c, *args, **kwargs):
        # 注意 train阶段过程中 t是在[0,timestep]范围中随机选取的
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
    ...
    def p_losses(self, x_start, cond, t, noise=None, ):
        # 对于inpaint任务，还需要保留融合ROI区域以外的图像信息，这里采用的方法是将背景图像分和mask图像与加噪图像在latent域进行维度拼接
        # cond条件（前景参考图像）作为外界条件输入attention部分
        if self.first_stage_key == 'inpaint':
            # x_start=x_start[:,:4,:,:]
            noise = default(noise, lambda: torch.randn_like(x_start[:,:4,:,:]))
            # 只对x_start的前四维进行扩散过程加噪， 也就是维度合并前的x_0 注意 这里是对encoder后的latent域中的groundtruth进行加噪
            x_noisy = self.q_sample(x_start=x_start[:,:4,:,:], t=t, noise=noise)
            # 将加噪完的x_t和原始的inpaint和mask特征进行合并，作为unet的输入
            x_noisy = torch.cat((x_noisy,x_start[:,4:,:,:]),dim=1)
        else:
            noise = default(noise, lambda: torch.randn_like(x_start))
            # 对x_0进行加噪，其中每次的t值变化，不等于timesteps
            x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        ...
        # 输出model_output为 1*4*64*64

        loss_dict = {}
        prefix = 'train' if self.training else 'val'

        if self.parameterization == "x0":
            target = x_start # x_0为groundtruth
        elif self.parameterization == "eps":
            target = noise
        else:
            raise NotImplementedError()

        # 注意 这里的loss计算是在unet的输入输出之间 也就是latent域中 和pixel域、encoder、decoder部分无关
